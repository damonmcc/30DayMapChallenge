...
file_path <- paste(script_dir, "/02 Lines - ", borough_name, " Random Spaces - ", Sys.time(), ".png", sep="")
...
pluto <- pluto %>% filter(borough == borough_initials, yearbuilt != 0)
...
upset(pluto, empty.intersections = "on", order.by = "freq")
# print(summary(pluto))
pluto %>%
  group_by(bldgclass) %>%
  summarise(n = n()) %>%
...
# upset(pluto,attribute.plots=list(plot=scatter_plot, x="bldgclass", y="yearbuilt"))
...
p1 <- ggplot() + 
  ...
  geom_point(data=pluto, aes(x=longitude, y=latitude, color=yearbuilt), alpha=point_alpha, size=point_size) +
  # scale_colour_gradientn(colours = cet_pal(5, name = "inferno", limits = c(1652, 2022)))
  scale_colour_gradientn(colours = cet_pal(5, name = "inferno"), limits = c(1652, 2022), breaks = c(1652, 1700, 1800, 1900, 2022)) +
